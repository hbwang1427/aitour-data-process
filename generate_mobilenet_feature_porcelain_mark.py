...
def main(ar):
    frozen_graph="/media/pangolins/Disc2/hbwang/aiguide/aitour/aiweb/assets/dl_models/porcelain_mark_256/porcelain_mark_256.pb"
    #frozen_graph="/home/pangolins/dl_models/porcelain_mark_256/porcelain_mark_mobilenetv1_256.pb"
    ...
    fpath = '/media/pangolins/Disc2/data/Porcelain/porcelain/Marks'
    dpath = '/media/pangolins/Disc2/data/Porcelain/porcelain/Marks/mobilenet_index_256.dat'
    ...
    with open(dpath,'wb') as fi:
      fi.write(struct.pack("<f", 256.0))
      #float_array = array('f', [512])
      #float_array.tofile(fi)
    for fd in os.listdir(fpath):
       if os.path.isdir(os.path.join(fpath,fd)):
         for subfd in os.listdir(os.path.join(fpath,fd)):
           if os.path.isdir(os.path.join(fpath,fd,subfd)):        
             files = [f for f in os.listdir(os.path.join(fpath,fd, subfd)) if os.path.isdir(os.path.join(fpath,fd,subfd))]        
             for f in files:
               if f.lower().endswith(('.png', '.jpg', '.jpeg', 'bmp')):
                 if f.find('small')==-1:
                   imname = os.path.join(fpath,fd,subfd,f)
